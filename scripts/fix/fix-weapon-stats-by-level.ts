  ...
  try {
    ...
    const rows = STAT_BREAKPOINTS.map(([level, ascension]) => {
      ...
      return {
        ...
        // BUG ĐÃ SỬA: trước đây ghi field "attack"/"specialized" (đúng
        // tên genshin-db trả về) nhưng WeaponLevelSlider.tsx (component
        // từng bị hỏng, đã viết lại từ đầu — xem comment đầu file đó)
        // lại đọc "baseAtk"/"subStatValue" — TÊN FIELD KHÔNG KHỚP NHAU
        // nên row.baseAtk/row.subStatValue luôn undefined, UI luôn hiện
        // "—" bất kể dữ liệu thật có tồn tại hay không. Đổi tên field ở
        // ĐÂY (nơi sinh dữ liệu) cho khớp đúng với type
        // WeaponStatByLevelRow mà component đang dùng, thay vì sửa
        // ngược lại component — vì "baseAtk"/"subStatValue" mới là tên
        // nhất quán với cột scalar Weapon.baseAtk/Weapon.subStatValue
...
async function main() {
  ...
  for (const w of weapons) {
    ...
    //
    // BUG ĐÃ SỬA: chỉ so độ dài mảng KHÔNG đủ — dữ liệu cũ ghi bởi phiên
    // bản trước của chính script này (field "attack"/"specialized" sai
    // tên) vẫn có ĐỦ số dòng, nên bị bỏ qua nhầm, không bao giờ được sửa
    // lại. Thêm điều kiện: nếu dòng đầu tiên đang lưu KHÔNG có field
    // "baseAtk" (tức dữ liệu cũ dùng tên field sai) thì vẫn phải ghi đè.
    ...
    const existingFirstRow = Array.isArray(w.statsByLevel) ? (w.statsByLevel as unknown[])[0] : null;
    const existingHasCorrectFieldNames =
      existingFirstRow !== null &&
      typeof existingFirstRow === "object" &&
      existingFirstRow !== null &&
      "baseAtk" in (existingFirstRow as Record<string, unknown>);
    if (existingCount === newCount && existingCount > 0 && existingHasCorrectFieldNames) continue;
