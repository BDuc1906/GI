...
function ReactionCard({
  ...
  return (
    <div className="relic-frame bg-bg-card border border-border rounded-xl p-4">
      ...
      <p className="text-sm text-text-secondary mb-2"><GlossaryText text={r.description} excludeId={r.id} /></p>
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {ELEMENTAL_RESONANCES.map((res) => (
            <div key={res.id} className="relic-frame bg-bg-card border border-border rounded-xl p-3">
              ...
              <p className="text-xs text-text-secondary"><GlossaryText text={res.description} excludeId={`resonance-${res.id}`} /></p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mb-3"><GlossaryText text={LUNAR_RESONANCE.description} /></p>
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mb-3"><GlossaryText text={HEXEREI_INFO.description} /></p>
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mb-3"><GlossaryText text={WITCH_REVELATION_INFO.description} /></p>
        <CharacterGroup label={t("versionLabel", { version: WITCH_REVELATION_INFO.gameVersion })} names={WITCH_REVELATION_CHARACTERS} />
 ...
 */
function CharacterGroup({ label, names }: { label: string; names: readonly string[] }) {
  return (
    <div className="mb-3">
...
const FORMULA_ORDER: DamageFormulaCategory[] = ["amplifying", "transformative", "additive", "lunarStellar"];
...
const FORMULA_TITLE_COLOR: Record<DamageFormulaCategory, string> = {
      ...
      {FORMULA_ORDER.map((cat) => {
        ...
        return (
          <div key={cat} className="relic-frame bg-bg-card border border-border rounded-xl p-4">
            <h3 className={`font-display font-bold mb-2 ${FORMULA_TITLE_COLOR[cat]}`}>{f.titleVi}</h3>
