    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col justify-center min-w-0">
            ...
            {(c.region || c.affiliation) && (
              <p className="text-xs text-text-muted mb-2 font-medium uppercase tracking-wider">
                {c.region && <>Vùng đất: <span className="text-text-primary">{c.region}</span></>}
                {c.region && c.affiliation && <span className="mx-2 text-border">·</span>}
                {c.affiliation && <>Phe phái: <span className="text-text-primary">{c.affiliation}</span></>}
              </p>
            )}

            {(c.birthday || c.constellationName || c.gameVersion) && (
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted mb-3">
                {c.birthday && <span>Sinh nhật: <span className="text-text-primary">{c.birthday}</span></span>}
                {c.constellationName && <span>Chòm sao: <span className="text-text-primary">{c.constellationName}</span></span>}
                {c.gameVersion && <span>Ra mắt phiên bản: <span className="text-text-primary">{c.gameVersion}</span></span>}
              </div>
            )}
      ...
      {statsByLevel.length > 0 && (
        <section className="mb-10">
          <SectionHeading elementColor={el}>Chỉ Số Theo Cấp</SectionHeading>
      ...
      {ascensionMaterials.length > 0 && (
        <section className="mb-10">
          <SectionHeading elementColor={el}>Nguyên Liệu Đột Phá</SectionHeading>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {ascensionMaterials.map((phase) => (
              <div key={phase.phase} className="surface-card p-4">
                <div className="text-eyebrow mb-3" style={{ color: el }}>Giai Đoạn {phase.phase}</div>
      ...
      {talents.length > 0 && (
        <section className="mb-10">
          <SectionHeading elementColor={el}>Kỹ Năng</SectionHeading>
          <div className="space-y-4">
            {talents.map((t) => (
              <div key={t.key} className="surface-card p-5">
                <div className="flex items-start gap-3 mb-2">
                  <span className="relative w-10 h-10 shrink-0 rounded skill-icon-frame border overflow-hidden">
                    {t.icon ? <SafeImage src={t.icon} alt={t.name ?? ""} fill className="object-contain p-1" sizes="40px" /> : null}
                  </span>
                  <div>
                    <div className="text-eyebrow">{TALENT_LABEL_VI[t.key] ?? t.key}</div>
                    <div className="font-display font-semibold text-base text-text-primary">{t.name}</div>
                  </div>
                </div>

                {t.description && (
                  <p className="text-sm text-text-secondary leading-relaxed whitespace-pre-line mb-3">{t.description}</p>
                )}
                ...
                {t.attributes && t.attributes.length > 0 && (
                  <div className="overflow-x-auto mb-1 rounded-lg border border-border">
                    <table className="w-full text-xs border-collapse">
                      <thead>
                        <tr>
                          <th className="py-1.5 px-3 text-left font-semibold whitespace-nowrap border-r border-border">Cấp Độ</th>
                          {Array.from({ length: Math.max(...t.attributes.map((r) => r.values.length)) }, (_, i) => i + 1).map((lv) => (
                ...
                {talentMaterials.length > 0 && ["normalAttack", "elementalSkill", "elementalBurst"].includes(t.key) && (
      ...
      {constellations.length > 0 && (
        <section className="mb-10">
          <SectionHeading elementColor="var(--rarity-4)">Hệ Thống Cung Mệnh Chòm Sao</SectionHeading>
          <div className="space-y-4">
