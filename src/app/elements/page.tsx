...
export const metadata: Metadata = {
  title: "Hệ thống Nguyên tố — LEIBO",
  description: "7 nguyên tố và toàn bộ phản ứng nguyên tố trong Genshin Impact: Bốc hơi, Tan chảy, Quá tải, Siêu dẫn, Cảm điện, Đóng băng, Khuếch tán, Kết tinh, Thiêu đốt, Sinh trưởng, Nảy mầm, Nở rộ, Xúc tác, Kích phát, Lan tỏa, Nguyệt, Tinh Tú Lấp Lánh.",
};
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <BreadcrumbJsonLd
        items={[
          { name: "LEIBO", path: "/" },
          { name: "Hệ thống Nguyên tố", path: "/elements" },
        ]}
      />

      <h1 className="text-3xl font-bold text-gold-bright mb-2">Hệ thống Nguyên tố</h1>
      <p className="text-secondary mb-8 max-w-2xl">
        7 nguyên tố (Vision) trong Genshin Impact và toàn bộ phản ứng khi kết hợp với nhau.
      </p>

      {/* 7 nguyên tố */}
      <section className="mb-10">
        <h2 className="font-display text-xl font-bold mb-4 text-gold border-b border-border pb-2">7 Nguyên tố</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
          {ELEMENTS.map((el) => (
            <a
              key={el.id}
              href={`#reactions-${el.id}`}
              className="relic-frame bg-card border border-border rounded-xl p-4 flex flex-col items-center gap-2 hover:border-gold/50 transition-colors"
            >
              <ElementIcon vision={el.name} iconUrl={ELEMENT_ICON_URLS[el.name]} size={40} />
              <div className="text-center">
                <div className="font-semibold text-primary text-sm">{el.name}</div>
                <div className="text-xs text-muted">{el.nameVi}</div>
              </div>
            </a>
          ))}
        </div>
      ...
      <section>
        <h2 className="font-display text-xl font-bold mb-4 text-gold border-b border-border pb-2">Tra theo nguyên tố</h2>
            ...
            return (
              <div key={el.id} id={`reactions-${el.id}`} className="scroll-mt-24">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-primary">{el.name} ({el.nameVi})</h3>
                </div>
